...
export const velyquaModules = [
  ...
  },
  {
    ...
    capabilities: [
      ...
      { id: 'platform.local-first', title: 'Local-first records', ownerValue: 'Commits a tank update and deterministic sync operation together before starting a network request.', status: 'working', audience: 'platform', evidence: ['Expo SQLite', 'account_tank_records', 'sync_outbox', 'saveTankRecordSqlite'] },
      ...
      { id: 'platform.offline-retry', title: 'Offline retry', ownerValue: 'Keeps account-scoped operations in a deterministic outbox and retries when connectivity returns.', status: 'working', audience: 'platform', evidence: ['sync_outbox', 'NetInfo', 'syncTankRecordWithRetry'] },
      ...
      { id: 'platform.data-deletion', title: 'In-app account deletion', ownerValue: 'Removes the owner account remotely and deletes only that account’s local record and outbox.', status: 'working', audience: 'platform', route: 'account', evidence: ['delete-account', 'removeUserTankDataSqlite'] },
