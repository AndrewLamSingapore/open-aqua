# VELYQUA Founding Tank

This is the first real aquarium used to shape and test VELYQUA. It belongs to VELYQUA's founder in Singapore.

## Confirmed baseline — 13 August 2026

- Indoor, planted freshwater community aquarium in Singapore
- Active for approximately six months
- Approximate owner measurements from tape photographs:
  - Length: 85 cm
  - Breadth: 40 cm
  - Height: 41 cm
- Geometric gross estimate: `85 × 40 × 41 ÷ 1,000 = 139.4 litres`

The 139.4-litre figure is **not** the actual water volume. Glass thickness, the water line, substrate, plants, hardscape and equipment reduce the amount of water. VELYQUA must not use this gross estimate for dosing until the working water volume is confirmed.

## Confirmed livestock groups

- Tetras — exact species and quantity pending
- Glass catfish — exact species and quantity pending
- Corys — exact species and quantity pending
- Rummy-nose tetras — exact species and quantity pending
- Killifishes — exact species and quantity pending
- Baby guppies — recently bred in this tank; quantity pending

VELYQUA stores unknown quantities as unknown. It does not convert a visible school in a photograph into a false count.

## Still needed from the owner

- Count for each livestock group
- Exact species or clear close photographs where identification is requested
- Current ammonia, nitrite, nitrate, pH and temperature readings
- Source-water readings
- Internal water depth or known fill volume
- Filter, light and other equipment models
- Plant inventory
- Feeding, testing, water-change and maintenance routines

Until the decision-critical water tests exist, Aqua Now should show **More information needed** rather than an invented safe status.
