...
async function authenticate(req) {
  ...
  if (!url || !key || !ownerId) {
    return { error: 'Owner bridge authentication is not configured.', status: 503 };
  }
...
async function primeRequest(config, path, init) {
  ...
  if (!response.ok) {
    const error = new Error('PRIME rejected the owner bridge request.');
...
module.exports = async function handler(req, res) {
  ...
  const routeValue = Array.isArray(req.query?.route)
    ? req.query.route[0]
    : req.query?.route;
  const route = String(routeValue || '');
  if (!['events', 'experiments'].includes(route)) {
    return json(res, 404, { error: 'Route not found.' });
  }
  if (
    (route === 'events' && req.method !== 'POST')
    || (route === 'experiments' && req.method !== 'GET')
  ) {
  ...
  if (!config) return json(res, 503, { error: 'PRIME bridge is not configured.' });

  try {
    if (route === 'experiments') {
      const payload = await primeRequest(
        config,
        '/api/integrations/portal/experiments',
        { method: 'GET' }
      );
      if (!payload || !Array.isArray(payload.experiments)) {
        throw new Error('PRIME experiment feed is malformed.');
      }
    ...
    if (!event || typeof event !== 'object' || Array.isArray(event)) {
      return json(res, 422, { error: 'A structured event is required.' });
    }
    if (event.account_id !== identity.user.id) {
      return json(res, 403, { error: 'Event account boundary mismatch.' });
    }
    if (!SAFE_EVENT_TYPES.has(event.event_type)) {
      return json(res, 422, { error: 'Unsupported observation-only event type.' });
    }
    if (!['experiment_execution', 'observation'].includes(event.aggregate_type)) {
      return json(res, 422, { error: 'Unsupported event aggregate.' });
    }
    if (JSON.stringify(req.body).length > 131072) {
      return json(res, 413, { error: 'Event is too large.' });
    }

    const payload = await primeRequest(
      config,
      '/api/integrations/velyqua/events',
    ...
    if (
      ...
      throw new Error('PRIME acknowledgement does not match the event.');
    }
    return json(res, 200, payload);
  } catch (error) {
    return json(res, 502, {
      error: 'PRIME owner bridge failed.',
